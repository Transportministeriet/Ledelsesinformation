...
KF_tal <- KF_bestand %>% 
  ...
  mutate(aar = parse_number(aar),
         ...
         oprindelse = "KF") %>% 
  filter(aar %in% c(as.numeric(nyeste_aar)-1, as.numeric(nyeste_aar))) %>% # TODO: Dette skal rettes når der kommer tal fra KF24
...
ny_figur_3_1 |> 
  ...
  hc_legend(reversed =TRUE) %>% 
  format_chart(titel =str_glue("<b>Figur 3.</b> Udviklingen i den samlede bestand af personbiler" ))

```


Udviklingen i salget af elbiler har resulteret i, at antallet af elbiler pr. 1. august 2023 overgik KF23's estimat for bestanden ved udgangen af 2023. Bestanden pr. `r str_remove_all(DST_tal$x_axis[[2]], " observeret")` er `r DST_tal$INDHOLD[[2]]` elbiler, jf. figur 4. 
...
Tilgangen af elbiler i 2023 var `r antal_el_biler_ialt` biler jf. figur 5. Dette er cirka `r pct_el_biler_ialt` pct. højere end estimeret i KF23 for samme periode.
...
data_fig_4 |> 
  ...
  hc_xAxis(title = list(text = NULL)) %>%
  format_chart(titel =str_glue("<b>Figur 5.</b> Udvikling i antallet af biler til bestanden fra januar til {format(ISOdate(2000, as.numeric(Nummer_sidste_md), 1), '%B')}" ))
