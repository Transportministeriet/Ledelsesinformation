...
filter(!str_detect(AAR, "9")) |># Exclude rows where AAR CONTATINS "9"
 ...
 format_chart("<b>Figur 2</b> Drivmidlernes andel af nyregistreringer", 
              note = "Kilde: Bilstatistik<br>*År til dato")
...
hc_legend(reversed = TRUE, 
 ...
 format_chart("<b>Figur 2.1</b> Drivmidlernes andel af nyregistreringer for lette lastbiler", 
              note = "Kilde: Bilstatistik<br>*År til dato<br>Note: Lette lastibler dækker over lastbiler med en på vægt på 4.250 kg. og nedefter.")
...
figur2_tunge <- nyreg_data_1_tunge_stjerne |> 
 ...
 hc_legend(reversed = TRUE, 
  ...
  format_chart("<b>Figur 2.2</b> Drivmidlernes andel af nyregistreringer for tunge lastbiler", 
              note = "Kilde: Bilstatistik<br>*År til dato<br>Note: Tunge lastbiler dækker over lastbiler med en vægt over 4.250 kg.")
...
I `r format(ISOdate(2000, nummer_sidste_md, 1), "%B")` måned voksede bestanden af ellastbiler med `r udv_elbestand_seneste_måned` lastbiler, således at der  ultimo `r format(ISOdate(2000, nummer_sidste_md, 1), "%B")` `r as.character(nyeste_aar)` var `r elbestand_seneste` ellastbiler i Danmark svarende til `r elandel_seneste` pct. af den samlede bestand jf. figur 3.


```{r}
#Ny figur 3 opgørelse af årlig bestand

bestand_data_1_stjerne |> 
  hchart('column', hcaes(x = AAR, y = ANTAL, group = DRIV ), borderColor = 0) |> 
  hc_plotOptions(column = list(stacking = "normal")) |> 
  hc_colors(c(trm_colors(farver))) |> 
  hc_yAxis(
...
figur3_lette <- bestand_data_1_lette_stjerne |> 
 ...
 pointFormat = "<b>{series.name}</b>: {point.percentage:.1f} pct. ({point.y:,.0f} lastbiler) <br>") %>%
  format_chart("<b>Figur 3.1</b> Udviklingen i den samlede bestand af lette lastbiler", 
              note = "Kilde: Bilstatistik<br>Note: Lette lastibler dækker over lastbiler med en på vægt på 4.250 kg. og nedefter.<br>*År til dato") %>% 
hc_legend(reversed = TRUE, 
            layout = "horizontal", 
            align = "center", 
            verticalAlign = "bottom",
...
figur3_tunge <-bestand_data_1_tunge_stjerne |> 
  hchart('column', hcaes(x = AAR, y = ANTAL, group = DRIV ), borderColor = 0) |> 
  hc_plotOptions(column = list(stacking = "normal")) |> 
  hc_colors(c(trm_colors(farver))) |> 
  hc_yAxis(
...
title = list(text = NULL),
 ...
 pointFormat = "<b>{series.name}</b>: {point.percentage:.1f} pct. ({point.y:,.0f} lastbiler) <br>") %>%
  format_chart("<b>Figur 3.2</b> Udviklingen i den samlede bestand af tunge lastbiler", 
              note = "Kilde: Bilstatisitk<br>Note: Tunge lastbiler dækker over lastbiler med en vægt over 4.250 kg.<br>*År til dato") %>%    
hc_legend(reversed = TRUE, 
            layout = "horizontal", 
            align = "center", 
            verticalAlign = "bottom",
...
bestand_data_1 %>% 
  ...
  format_chart(str_glue("<b>Figur 4.</b> Status for antallet af ellastbiler i {nyeste_aar} - oberserveret versus forventet"),
...
bestand_data_udv |>
  ...
  hc_tooltip() %>% 
  format_chart(str_glue("<b>Figur 5.</b> Udviklingen i bestand af lastbiler fordelt på drivmidler. Januar - {seneste_md}"), note="Kilde: Bilstatisitk")
...
fct_sort_underkat <- nyreg_data_underkat$AAR %>% unique()
...
map(udvalgte_kat, ~{nyreg_data_underkat |>
  ...
  hc_xAxis(categories = fct_sort_underkat, title = list(text = NULL),
      ...
      formatter = JS("
        function() {
          var categories = this.axis.categories; // Access explicitly set categories
          var secondToLastIndex = categories.length - 2;
  ...
  format_chart(str_glue("<b>{str_replace_all(.x, c('_'=' ', 'OE'='Ø', 'AA'='Å', 'AE'='Æ')) %>% str_to_sentence()}<b>"), note="Kilde: Bilstatistik<br>*År til dato")}) |> 
...
map(udvalgte_kat, ~{bestand_data_underkat |> 
 ...
 hc_legend(
  ...
  itemDistance = 1,             # Adjust space between items
    ...
    itemStyle = list(
      fontSize = "12",
          width = 60              # Set an appropriate font size
    )              # Set the legend width to fit 2 items per row)# Reduce space between legend items# Place legends at the bottom
  )%>% 
  format_chart(str_glue("<b>{str_replace_all(.x, c('_'=' ', 'OE'='Ø', 'AA'='Å', 'AE'='Æ')) %>% str_to_sentence()}<b>"),note="Kilde: Bilstatisitk<br>*År til dato")}) |> 
  hw_grid(rowheight = 400) %>% htmltools::browsable()
