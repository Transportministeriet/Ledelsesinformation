...
bestand_data_udv <- bestand_data_samlet_gammel_metode %>%
  ...
  arrange(DRIV, DATE) %>%
  group_by(DRIV) %>%
  mutate(lag_antal = lag(ANTAL, n = as.numeric(nummer_sidste_md)), UDV = ANTAL - lag_antal) %>%
  ungroup() %>%
...
#   mutate(AAR = ifelse(month(DATE) != 12, year(DATE), year(DATE) + 1)) %>%
#   arrange(DRIV, AAR, DATE) %>%  # Ensure correct order before lag()
#   group_by(DRIV, AAR) %>%  # Group by both DRIV and AAR
#   mutate(lag_antal = lag(ANTAL, order_by = DATE)) %>%  # Ensure lag follows correct order
...
Ifølge Klimastatus og -fremskrvining 2024 forventes der at være cirka `r indev_aar_est` elektriske lastbiler i Danmark ved udgangen af `r as.character(nyeste_aar)`. Differencen mellem det nuværende antal, jævnfør figur 3, og det forventede antal elektriske lastbiler ved udgangen af `r as.character(nyeste_aar)`, jævnfør figur 4, er således `r rest` lastbiler.
