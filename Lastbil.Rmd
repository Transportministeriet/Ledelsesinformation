...
bestand_data_underkat <- bestand_data_samlet_gammel_metode %>% 
...
bestand_seneste_lette <- bestand_data_1_lette %>% filter(str_detect(AAR, "\\(")) %>% pull("ANTAL") %>% sum()
bestand_seneste_tunge <- bestand_data_1_tunge %>% filter(str_detect(AAR, "\\(")) %>% pull("ANTAL") %>% sum()
elbestand_seneste <- bestand_data_1 %>% filter(str_detect(AAR, "\\("), DRIV=="El") %>% pull("ANTAL")
elbestand_seneste_lette <- bestand_data_1_lette %>% filter(str_detect(AAR, "\\("), DRIV=="El") %>% pull("ANTAL")
elbestand_seneste_tunge <- bestand_data_1_tunge %>% filter(str_detect(AAR, "\\("), DRIV=="El") %>% pull("ANTAL")
...
```{r}
list("Figur 1 og 2 samlet" = figur_1_2, "Figur 1 og 2 lette" = figur_1_2_lette, "Figur 1 og 2 tunge" = figur_1_2_tunge, "Figur 3 samlet" = figur_3,"Figur 3 lette" = figur_3_lette, "Figur 3 tunge" = figur_3_tunge, "Figur 4" = figur_4, "Figur 5" = figur_5,
...
nyreg_data_1 |> 
  ...
  # hc_subtitle(
  #           point = list(x = 0.5, y = 0.0, xAxis = 0, yAxis = 0), # Position relative to chart
  #   text =  "Kilde: Bilstatistik",
  #   align = "bottom",
  #    verticalAlign = "bottom",  # Place at the bottom of the chart area
  #   y = 20,  # Adjust this value to fine-tune the position below the legend
  #   style = list(fontSize = "12px", fontStyle = "italic")
  # ) %>%
  format_chart("<b>Figur 1.</b> Udvikling i nyregistreringer af lastbiler fordelt på drivmiddel", "Kilde: Bilstatistik")
...
formatted_text <- div(
 style = sprintf("background-color: %s; padding: 5px 20px; border: 1px solid #ddd; 
    ...
    Lastbiler med en vægt over 4.250 kg. defineres som tunge lsatbiler. Det bør bemærkes, at der i andre sammenhæng kan forekomme andre definitnioner af henholdsvis lette og tunge lastibler."))
  
  
browsable(formatted_text)
```
Det er primært de lette lastbiler, der omstilles til eldrift. I `r as.character(nyeste_aar)`  udgjorde ellastbiler `r elsalg_seneste_lette/totalsalg_seneste_lette*100` pct. af alle nyregistreringer inden for kategorien lette lastbiler, jf. figur 2.1. For tunge lastbiler udgjorde ellastbiler i samme periode kun `r elsalg_seneste_tunge/totalsalg_seneste_tunge*100` pct. af nyregistreringerne, jf. figur 2.2.
...
hc_legend(reversed = TRUE, 
            ...
            itemDistance = 2 )%>% 
  hc_chart(marginBottom = 176)  %>%
 hc_tooltip(shared = TRUE,
  pointFormat = "<b>{series.name}</b>: {point.percentage:.1f} pct. (<b>{point.antal:,.0f}</b> lastbiler)<br>") %>%
 format_chart("<b>Figur 2.1</b> Drivmidlernes andel af nyregistreringer for lette lastbiler", 
              note = "Note: Lette lastibler dækker over lastbiler med en på vægt på 4.250 kg. og nedefter. Tunge lastbiler dækker over lastbiler med en vægt over 4.250 kg.")
...
figur2_tunge <- nyreg_data_1_tunge |> 
 ...
 hc_legend(reversed = TRUE, 
    ...
    hc_chart(marginBottom = 160)  %>%# Reduce font size if necessary %>%  # Adding spacing between items
  hc_tooltip(shared = TRUE,
  pointFormat = "<b>{series.name}</b>: {point.percentage:.1f} pct. (<b>{point.antal:,.0f}</b> lastbiler)<br>") %>%
  format_chart("<b>Figur 2.2</b> Drivmidlernes andel af nyregistreringer for tunge lastbiler", 
              note = "Note: Lette lastibler dækker over lastbiler med en på vægt på 4.250 kg. og nedefter. Tunge lastbiler dækker over lastbiler med en vægt over 4.250 kg.")



#<br>  Kilde: Bilstatistik bør indsættes i text på et senere tidspunkt 
...
Diesel er stadig det dominernde drivmiddel i bestanden af lastbiler. Bestanden af lastbiler har generelt været svagt stigende siden 1. januar 2021, hvor den samlede bestand var `r bestand_2021`, mens  bestanden ultimo `r format(ISOdate(2000, nummer_sidste_md, 1), "%B")` `r as.character(nyeste_aar)` var `r bestand_seneste` lastbiler. 

Omend bestanden af lette lastbiler blot udgør `r lette_andel_af_bestand` pct. af den samlede bestand af lastbiler, er det værd at bemærke, at bestanden af elastbiler er i vækst, og at det er særligt er lette ellastbiler, der kommer til. Ellastbiler udgjorde ved udgangen af oktober måned `r elbestand_seneste_lette/bestand_seneste_lette*100` pct. af den samlede bestand af lette lastbiler, jævnfør figur 3.1. I modsætning hertil udgjorde ellastbiler blot `r elbestand_seneste_tunge/bestand_seneste_tunge*100` pct. af den samlede bestand af tunge lastbiler, jævnfør figur 3.2. Lette elastbiler udgør således `r ellette_andel_af_elbestand` pct. af den samlede bestand af ellastbiler. 
...
figur3_lette <-bestand_data_1_lette |> 
  ...
  hc_tooltip(shared=TRUE,
 pointFormat = "<b>{series.name}</b>: {point.percentage:.1f} pct. ({point.y:,.0f} lastbiler) <br>") %>%
  format_chart("<b>Figur 3.1</b> Udviklingen i den samlede bestand af lette lastbiler", 
              note = "Note: Lette lastibler dækker over lastbiler med en på vægt på 4.250 kg. og nedefter. Tunge lastbiler dækker over lastbiler med en vægt over 4.250 kg.") %>% 
hc_legend(reversed = TRUE, 
            layout = "horizontal", 
            align = "center", 
...
figur3_tunge <-bestand_data_1_tunge |> 
  ...
  hc_xAxis(title = list(text = NULL),
    ...
    hc_tooltip(shared=TRUE,
 pointFormat = "<b>{series.name}</b>: {point.percentage:.1f} pct. ({point.y:,.0f} lastbiler) <br>") %>%
  format_chart("<b>Figur 3.2</b> Udviklingen i den samlede bestand af tunge lastbiler", 
              note = "Note: Lette lastibler dækker over lastbiler med en på vægt på 4.250 kg. og nedefter. Tunge lastbiler dækker over lastbiler med en vægt over 4.250 kg.") %>%    
hc_legend(reversed = TRUE, 
            layout = "horizontal", 
            align = "center", 
...
I `r format(ISOdate(2000, nummer_sidste_md, 1), "%B")` måned voksede bestanden af ellastbiler med `r udv_elbestand_seneste_måned` lastbiler. Jævnfør Klimastatus og -fremskrvining 2024 forventes der at være cirka `r KF_tal$ANTAL[[2]]` elektriske lastbiler i Danmark ved udgangen af 2025. Differencen mellem det nuværende  og det forventede antal elektriske lastbiler ved udgangen af 2025 er således `r rest` lastbiler, jævnfør figur 4.
