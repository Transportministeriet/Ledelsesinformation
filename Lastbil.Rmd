...
sti <- "S:/TRM Databank/004 Bilstatistik/Lastbil"
  ...
  dir(sti) %>%
    ...
    map_dfr(~read_excel(str_c(sti, "/", .x), skip = 9,
              ...
              mutate(
                ...
                DATE = str_remove_all(.x, "[-.]") %>%  parse_number(.x),
                DATE = dmy(DATE)
              ) %>%
              select(-DRIVKRAFT)
    ) %>%
...
bestand_data_udv <- bestand_data_samlet_gammel_metode %>%
  ...
  mutate(AAR = year(DATE)) %>%
  arrange(DRIV, DATE) %>%
  group_by(DRIV) %>%
  mutate(lag_antal = lag(ANTAL, n = as.numeric(nummer_sidste_md)), 
         UDV = ANTAL - lag_antal) %>%
  ungroup() %>%
  filter(month(DATE) == nummer_sidste_md, AAR > 2019, !is.na(lag_antal)) %>%
  mutate(DRIV = factor(DRIV, levels = c("El", "Gas", "Diesel", "Øvrige drivmidler", "Total"))) %>%
  select(-KASSORI) %>%
  distinct(AAR, DRIV, .keep_all = TRUE) %>%
  arrange(AAR)
  ...
  data %>% 
    ...
    (\(y) bind_rows(
      ...
      summarise(y, ANTAL = sum(ANTAL), .by = c("AAR", "DRIV")) %>% 
        mutate(AAR = ifelse(AAR == as.character(nyeste_aar), paste0(AAR, stjerne), AAR)),
...
KF_tal <- fread(str_c(sti_kf, "/tidy_datasaet/", "bestand_kf26.csv")) %>% 
  filter(type == "Lastbiler", drivmiddel == "El") %>% 
  summarise(vaerdi = sum(vaerdi),
    .by = c("aar", "drivmiddel")) %>% 
  mutate(aar = as.numeric(aar),
         x_axis = str_glue("Forventet {aar} (KF26)"),
         oprindelse = "KF") %>% 
  filter(aar %in% c(as.numeric(nyeste_aar), as.numeric(nyeste_aar) + 1)) %>%  # TODO: Adjust when KF25 data is available
  select(ANTAL = vaerdi, x_axis, oprindelse)
...
nyreg_data_underkat <- nyreg_data_uden_vægt %>% 
  ...
  (\(y) bind_rows(summarise(y, ANTAL = sum(ANTAL), 
                  ...
                  filter(y, DATE_SL==max(DATE_SL)) %>% 
                    mutate(AAR = str_c(format(DATE_SL, "%B"), " ", year(DATE_SL))) %>% 
...
totalsalg_seneste <- nyreg_data_1 %>% filter(str_detect(AAR, str_c("^", nyeste_aar))) %>% pull(ANTAL) %>% sum()
elsalg_seneste <- nyreg_data_1 %>% filter(str_detect(AAR, str_c("^", nyeste_aar)), DRIV=="El") %>% pull(ANTAL) %>% sum()


totalsalg_seneste_lette <- nyreg_data_1_lette %>% filter(str_detect(AAR, str_c("^", nyeste_aar))) %>% pull(ANTAL) %>% sum()
elsalg_seneste_lette <- nyreg_data_1_lette %>% filter(str_detect(AAR, str_c("^", nyeste_aar)), DRIV=="El") %>% pull(ANTAL) %>% sum()


totalsalg_seneste_tunge <- nyreg_data_1_tunge %>% filter(str_detect(AAR, str_c("^", nyeste_aar))) %>% pull(ANTAL) %>% sum()
elsalg_seneste_tunge <- nyreg_data_1_tunge %>% filter(str_detect(AAR, str_c("^", nyeste_aar)), DRIV=="El") %>% pull(ANTAL) %>% sum()
...
seneste_md_tal <- nyreg_data_1 %>%
  filter(AAR == max(AAR), DRIV == "El") %>%
  pull("A_NYREG")

bestand_2021 <- bestand_data_1 %>% filter(AAR=="2020") %>% pull("ANTAL") %>% sum()

bestand_seneste <- bestand_data_1 %>%
  filter(if (any(!str_detect(AAR, "^\\d{4}$"))) {
    !str_detect(AAR, "^\\d{4}$")
  } else {
    AAR == as.character(nyeste_aar)
  }) %>%
  pull("ANTAL") %>%
  sum()

bestand_seneste_lette <- bestand_data_1_lette %>%
  filter(if (any(!str_detect(AAR, "^\\d{4}$"))) {
    !str_detect(AAR, "^\\d{4}$")
  } else {
    AAR == as.character(nyeste_aar)
...
bestand_seneste_tunge <- bestand_data_1_tunge %>%
  filter(if (any(!str_detect(AAR, "^\\d{4}$"))) {
    !str_detect(AAR, "^\\d{4}$")
  } else {
    AAR == as.character(nyeste_aar)
...
elbestand_seneste_lette <- bestand_data_1_lette %>%
  filter(if (any(!str_detect(AAR, "^\\d{4}$"))) {
    !str_detect(AAR, "^\\d{4}$")
  } else {
    AAR == as.character(nyeste_aar)
...
elbestand_seneste_tunge <- bestand_data_1_tunge %>%
  filter(if (any(!str_detect(AAR, "^\\d{4}$"))) {
    !str_detect(AAR, "^\\d{4}$")
  } else {
    AAR == as.character(nyeste_aar)
...
elbestand_næst_seneste <- bestand_data_2 %>%
  filter(!str_detect(AAR, "^\\d{4}$"), DRIV == "El") %>%
  pull("ANTAL")
...
figur_4 <- bestand_data_1 %>%
  filter(AAR %in% c(as.character(nyeste_aar - 1), as.character(nyeste_aar)), DRIV == "El") %>%
  mutate(
    x_axis = ifelse(
      AAR == as.character(nyeste_aar - 1),
      str_c("Ultimo ", nyeste_aar - 1),
      str_c("Ultimo ", seneste_md, " ", nyeste_aar) # Ensure ISO_data$month exists
    ),
    oprindelse = "Bilstatistik",
    .keep = "unused"
  ) %>%
  bind_rows(KF_tal) %>%
  select(kilde = oprindelse, dato = x_axis, antal = ANTAL)


## GAMMEL## 
# figur_4 <- bestand_data_1 %>%
#   filter(AAR %in% c(nyeste_aar - 1, str_c(seneste_md, " ", nyeste_aar)), DRIV == "El") %>%
#   mutate(
#     x_axis = ifelse(
#       AAR == nyeste_aar - 1,
#       str_c("Ultimo ", nyeste_aar - 1),
...
farver <- c("roed", "blaa", "orange", "groen") %>% factor(., levels = .)

nyreg_data_1 %>%
  filter(AAR >= (nyeste_aar-5)) %>% 
  mutate(AAR = ifelse(AAR == nyeste_aar, paste0(AAR, stjerne), AAR)) |>
  (\(y)
   hchart(y, 'column', hcaes(x = AAR, y =ANTAL, group = DRIV ), borderColor = 0) %>%
     hc_plotOptions(column = list(stacking = "normal")) |> 
     hc_colors(c(trm_colors(farver, lys = ifelse(farver == "orange", 2, 0)))) |> 
     hc_yAxis(title = list(text = "Antal"),
              labels = list(format = "{value:,.0f}"),
              stackLabels = list(enabled = TRUE, verticalAlign = 'top',
                                 crop = FALSE, overflow = 'none')) |> 
     hc_xAxis(title = list(text = NULL),
              categories = distinct(y, AAR) %>% deframe()
     ) %>% 
...
fct_sort_stjerne <- nyreg_data_1$AAR %>% unique()
...
formatted_text <- div(
 style = sprintf("background-color: %s; padding: 5px 20px; border: 1px solid #ddd; 
    ...
    Lastbiler med en vægt over 4.250 kg. defineres som tunge lastbiler. Det bør bemærkes, at der i andre sammenhænge kan forekomme andre definitioner af henholdsvis lette og tunge lastibler."))
...
fct_sort <- nyreg_data_1_lette$AAR %>% unique()

figur2_lette <- nyreg_data_1_lette |>
  filter(!str_detect(AAR, "9")) |># Exclude rows where AAR CONTATINS "9"
  ...
  hc_colors(as.character(trm_colors(farver[c(2, 4)]))) |>  # Use only "cyan" and "groen" (the second and fourth colors)
  hc_yAxis(title = list(text = "Pct."), max = 100) |> 
...
figur2_tunge <- nyreg_data_1_tunge |>
  filter(!str_detect(AAR, "9")) |># Exclude rows where AAR CONTATINS "9"
  mutate(AAR = factor(AAR, levels = fct_sort),
         antal = ANTAL) %>%  # Add ANTAL column for use in the tooltip
  hchart('column', hcaes(x = AAR, y = A_NYREG , group = DRIV), borderColor = 0) |> 
...
Diesel er stadig det dominerende drivmiddel i bestanden af lastbiler. Bestanden af lastbiler har generelt været svagt stigende siden ultimo 2020, hvor den samlede bestand var `r bestand_2021`, mens  bestanden ultimo `r str_c(seneste_md, " ", nyeste_aar)` var `r bestand_seneste` lastbiler. 

I `r seneste_md` måned udviklede bestanden af ellastbiler sig med `r udv_elbestand_seneste_måned` lastbiler således, at der ultimo `r str_c(seneste_md, " ", nyeste_aar)` var `r elbestand_seneste` ellastbiler i Danmark svarende til `r elandel_seneste` pct. af den samlede bestand jf. figur 3.
...
figur3_lette <- bestand_data_1_lette |> 
  hchart('column', hcaes(x = AAR, y = ANTAL, group = DRIV), borderColor = 0) |> 
  hc_plotOptions(column = list(stacking = "normal")) |> 
  hc_colors(as.character(trm_colors(farver[c(1, 2, 4)]))) |>  # Use only "cyan" and "groen" (the second and fourth colors)
  hc_yAxis(
    title = list(text = "Antal"),
    labels = list(formatter = JS("function() {
      return this.value >= 1000 ? (this.value / 1000).toFixed(0) : this.value;
  ...
  trm_hc_format("<b>Figur 3.1</b> Udviklingen i den samlede bestand af lette lastbiler",
                note = str_glue("Kilde: Bilstatistik<br>Note 1: Lette lastibler dækker over lastbiler med en på vægt på 4.250 kg. og nedefter.<br>Note 2: Bestanden er opgjort ultimo"))%>% 
  hc_legend(reversed = TRUE, 
            layout = "horizontal", 
            align = "center", 
            verticalAlign = "bottom",
...
figur3_tunge <-bestand_data_1_tunge |> 
  ...
  trm_hc_format("<b>Figur 3.2</b> Udviklingen i den samlede bestand af tunge lastbiler",
                note = str_glue("Kilde: Bilstatisitk<br>Note 1: Tunge lastbiler dækker over lastbiler med en vægt over 4.250 kg.<br>Note 2: Bestanden er opgjort ultimo"))%>%   
  hc_legend(reversed = TRUE, 
            layout = "horizontal", 
            align = "center", 
            verticalAlign = "bottom",
...
indev_aar_est <- KF_tal %>% filter(str_detect(x_axis, as.character(nyeste_aar))) %>% pull(ANTAL)
rest <- abs(indev_aar_est - elbestand_seneste)

```


Ifølge Klimastatus og -fremskrivning 2026 forventes der at være cirka `r indev_aar_est` elektriske lastbiler i Danmark ved udgangen af `r as.character(nyeste_aar)`. Differencen mellem det nuværende antal, jævnfør figur 3, og det forventede antal elektriske lastbiler ved udgangen af `r as.character(nyeste_aar)`, jævnfør figur 4, er således `r rest` lastbiler.


```{r}
#Figur 4. Status for antallet af ellastbiler i 2026 - oberserveret versus forventet

## NY ##
bestand_data_1 %>% 
  filter(AAR %in% c(nyeste_aar, nyeste_aar-1) | AAR == str_c(seneste_md, " ", nyeste_aar), DRIV == "El") %>% 
  mutate(x_axis = AAR,
         oprindelse = "Bilstatistik",
         ANTAL,
         .keep = "none") %>%
  ...
  hc_xAxis(
    title = list(text = NULL),
    labels = list(align = "center",    rotation = 0),tickmarkPlacement = "on") |>
  ...
  trm_hc_format("<b>Figur 4.</b> Status for antallet af ellastbiler i 2026",
                note = tekst_note_bestand_bilstat)

## GAMMEL ##

# bestand_data_1 %>% 
#   filter(AAR == nyeste_aar-1 | AAR == str_c(seneste_md, " ", nyeste_aar), DRIV == "El") %>% 
#   mutate(x_axis = AAR,
#          oprindelse = "Bilstatistik",
#          ANTAL,
#          .keep = "none") %>%
#   bind_rows(KF_tal) |> 
#   hchart('column', hcaes(x = x_axis, y = ANTAL, group = oprindelse), 
#          color = trm_colors(c("cyan", "orange")), borderColor = 0) |>
#   hc_yAxis(title = list(text = "Antal")) |>
#   hc_xAxis(
#     title = list(text = NULL),
#     
...
#     pointPadding = 0.1    
#   )) %>%
#   trm_hc_format("<b>Figur 4.</b> Status for antallet af ellastbiler i 2025",
#                 note = tekst_note_bestand_bilstat)
...
bestand_data_udv |>
  ...
  hc_colors(c(trm_colors(
  c(sort(farver, decreasing = T) %>% as.character(), "blaa"),
  lys = c(ifelse(sort(farver, decreasing = T) %>% as.character() == "orange", 2, 0), 1)))) |>   # orange (gas) = 2, ekstra "blaa" = 1 (undgår kollision med farvers egen "blaa")
  # hc_legend(reversed =TRUE) %>%
  hc_yAxis(title = list(text = "Antal"),
           min = 0,
           labels = list(
...
Bestanden af gasrenovationslastbiler toppede i 2021 og har været svagt faldende siden. I samme periode er ellastbiler gået fra at udgøre 2,1 pct. ultimo 2021 til at udgøre `r bestand_data_underkat_txt_el$RENOVATIONSAGGREGAT` pct. i `r seneste_md` `r as.character(nyeste_aar)`.
...
map(udvalgte_kat, ~{
  bestand_data_underkat |> 
    ...
    trm_hc_format(
      ...
      note = "Kilde: Bilstatistik<br>Note: Bestanden er opgjort ultimo"
    )
}) |> 
  hw_grid(rowheight = 400) %>% htmltools::browsable()
