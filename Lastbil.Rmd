...
bestand_data_udv <- bestand_data_samlet_gammel_metode %>% 
  ...
  arrange(DRIV, DATE) %>% 
  mutate(UDV = ANTAL - lag(ANTAL),
         .by = "DRIV") %>% 
  filter(AAR > 2019, month(DATE)==nummer_sidste_md)
...
bestand_data_1 %>% 
  ...
  trm_hc_format(str_glue("<b>Figur 4.</b> Status for antallet af ellastbiler i {nyeste_aar} - oberserveret versus forventet"),
...
bestand_data_udv |>
  ...
  hc_tooltip() %>% 
  trm_hc_format(str_glue("<b>Figur 5.</b> Udviklingen i bestand af lastbiler fordelt på drivmidler. Januar - {seneste_md}"), note="Kilde: Bilstatisitk")
...
map(udvalgte_kat, ~{nyreg_data_underkat |>
  ...
  trm_hc_format(str_glue("<b>{str_replace_all(.x, c('_'=' ', 'OE'='Ø', 'AA'='Å', 'AE'='Æ')) %>% str_to_sentence()}<b>"), note="Kilde: Bilstatistik<br>*År til dato")}) |> 
...
map(udvalgte_kat, ~{bestand_data_underkat |> 
 ...
 hc_legend(
  ...
  trm_hc_format(str_glue("<b>{str_replace_all(.x, c('_'=' ', 'OE'='Ø', 'AA'='Å', 'AE'='Æ')) %>% str_to_sentence()}<b>"),note="Kilde: Bilstatisitk<br>*År til dato")}) |> 
  hw_grid(rowheight = 400) %>% htmltools::browsable()
