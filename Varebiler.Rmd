...
Dette er Transportministeriets interne statusnotat for nyregistreringer og bestanden af varebiler. Det viser bl.a. udviklingen i antallet af varebiler fordelt på drivmiddeltyper. Notatet opdateres månedligt og er baseret på data fra Danmarks Statistik.  
...
expr <- parse(text = str_c(
  "map(sidste_par_aar$DRIV |> unique(), 
    ~{hchart(sidste_par_aar |> filter(DRIV == .x), 'line', hcaes(x = month.abb[MD], y = UDV_BIL, group = TYPE ), marker = FALSE) |> 
        hc_colors(c(trm_colors(c('graa', 'orange')))) |> 
...
Varebiler kan opdeles efter brug, afhængigt af om de bruges i husholdningen eller i erhvervet. Foreløbigt i `r as.character(nyeste_aar)` udgjorde nyregistreringer i husholdningen `r round(totalsalg_hus/totalsalg * 100,1)` pct. af alle nyregistreringer, mens erhvervet stod for `r round(totalsalg_erhverv/totalsalg * 100,1)` pct. af nyregistreringerne.

Opdeles varebiler efter brug, fremgår det, at det primært er varebiler i erhvervet, der omstilles til eldrift. Foreløbigt i `r as.character(nyeste_aar)`  har el-varebiler udgjort `r round(eltotalsalg_hus/totalsalg_hus * 100, 1)` pct. af nyregistreringerne i husholdningen, jf. figur 7.1. I samme periode udgjorde el-varebiler i erhvervet `r round(eltotalsalg_erhverv/totalsalg_erhverv * 100, 1)` pct. af nyregistreringerne, jf. figur 7.2.

```{r}
#Figur 7.1 Husholdningens andel af nyregistreringer for varebiler
fct_sort_hus <- data_nysalg_hus_1$AAR %>% unique()

figur7.1 <- data_nysalg_hus_1 |>
  ...
  hc_tooltip(shared = TRUE,             pointFormat = "<b>{series.name}</b>: {point.percentage:.1f} pct. (<b>{point.antal:,.0f}</b> lastbiler)<br>") %>%
  trm_hc_format("<b>Figur 7.1</b> Husholdningens andel af nyregistreringer for varebiler",
                note = str_glue("Kilde: Danmarks Statistik <br> {stjerne_tekst}"))


#Figur 7.2 Erhvervets andel af nyregistreringer for varebiler

fct_sort_erhverv <- data_nysalg_erhverv_1$AAR %>% unique()

figur7.2 <- data_nysalg_erhverv_1 |>
  filter(!AAR %in% c("2018", "2019"), DRIV != "Øvrige drivmidler") |># Exclude rows where AAR CONTATINS "9"
  ...
  trm_hc_format("<b>Figur 7.2</b> Erhvervets andel af nyregistreringer for varebiler",
                note = str_glue("Kilde: Danmarks Statistik <br> {stjerne_tekst}"))
...
Bestanden af varebiler i husholdningen udgør `r round(totalbestand_hus/totalbestand * 100, 1)` pct. af den samlede bestand af varebiler, men bestanden af varebiler i erhvervet udgør `r round(totalbestand_erhverv/totalbestand * 100, 1)` pct.

Bestanden af varebiler i husholdningerne er faldet med `r abs(round((totalbestand_hus - totalbestand_20_hus)/totalbestand_20_hus * 100, 1))` pct. siden 2020, jf. figur 8.1. Herudover ses det, at el-varebiler næsten ingen andel af bestanden af varebiler i husholdningen, mens bestanden af fossil-elvarebiler udgør `r round(fostotalbestand_hus/totalbestand_hus * 100, 1)` pct. Bestanden af varebiler i erhvervet er nogenlunde konstant siden 2020, jf. figur 8.2, hvor bestanden er vokset med `r totalbestand_erhverv - totalbestand_20_erhverv` varebiler. Endeligt ses der en vækst i andelen af el-varebiler i erhvervet. Foreløbigt i `r as.character(nyeste_aar)` udgør de `r round(eltotalbestand_erhverv/totalbestand_erhverv * 100, 1)`  pct. af den samlede bestand i erhvervet, mens fossil-varebiler udgør `r round(fostotalbestand_erhverv/totalbestand_erhverv * 100, 1)` pct.
```{r}
#Figur 8.1. Udviklingen i den samlede bestand af varebiler i husholdningen

categories_hus <- unique(data_bil_hus_1_ny$AAR)

figur8.1 <- data_bil_hus_1_ny |> 
  ...
  hc_tooltip(shared=TRUE,
             pointFormat = "<b>{series.name}</b>: {point.percentage:.1f} pct. ({point.y:,.0f} varebiler) <br>") %>%
  trm_hc_format("<b>Figur 8.1</b> Udviklingen i den samlede bestand af varebiler i husholdningen",
                note = str_glue("Kilde: Danmarks Statistik"))%>% 
  hc_legend(reversed = TRUE, 
            layout = "horizontal", 
            align = "center", 
...
#Figur 8.2. Udviklingen i den samlede bestand af varebiler i husholdningen

categories_erhverv <- unique(data_bil_erhverv_1_ny$AAR)

figur8.2 <- data_bil_erhverv_1_ny |> 
  hchart('column', hcaes(x = AAR, y = ANTAL, group = DRIV), borderColor = 0) |> 
  hc_plotOptions(column = list(stacking = "normal")) |> 
  hc_colors(as.character(trm_colors(farver[c(1, 2, 3, 4)]))) |> 
  hc_yAxis(
  ...
  hc_tooltip(shared=TRUE,
             pointFormat = "<b>{series.name}</b>: {point.percentage:.1f} pct. ({point.y:,.0f} varebiler) <br>") %>%
  trm_hc_format("<b>Figur 8.2</b> Udviklingen i den samlede bestand af varebiler i erhvervet",
                note = str_glue("Kilde: Danmarks Statistik"))%>% 
  hc_legend(reversed = TRUE, 
            layout = "horizontal", 
            align = "center", 
...
html2 <- tagList(
  div(style = "display: flex; justify-content: space-between;",
      div(style = "width: 48%;", figur8.1),
      div(style = "width: 48%;", figur8.2)
  )
)
