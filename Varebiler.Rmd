...
dat_fig_3 <- tibble("AAR"=round(sort(rep(2022:2026, 12)) + rep((c(1:12)-1)/12, 5), 3)) |> 
  ...
  inner_join(data_hush_erhv_1  |> 
               filter( BRUG=="I alt", DRIV=="El") |> 
...
Den 1. januar 2020 var den samlede bestand `r totalbestand["I alt 2019"]` varebiler. Fra januar 2020 til `r t_sidste_md` `r nyeste_aar` er bestanden faldet med `r abs(totalbestand[str_glue("I alt {senest_md_tekst}")]/totalbestand["I alt 2019"]-1)*100` pct., så den danske bestand af varebiler ultimo `r t_sidste_md` var `r totalbestand[str_glue("I alt {senest_md_tekst}")]` varebiler. I `r t_sidste_md` `r nyeste_aar` udgjorde elvarebiler `r round((elbestand[str_glue("I alt {senest_md_tekst}")]/totalbestand[str_glue("I alt {senest_md_tekst}")])*100,1)` pct. af den samlede bestand.
...
Alt imens bestanden af elvarebiler er under en positiv udvikling, falder bestanden af fossilvarebiler fortsat. I de første `r month(max(dat_fig_3$DATE))` måneder af 2020 var der en nettoafgang på `r abs(tal_fos_2019)`, hvor der i de første `r month(max(dat_fig_3$DATE))` måneder af `r nyeste_aar` var en nettoafgang på `r abs(tal_fos_2023)` fossilvarebiler.

Af figuren fremgår det yderligere, at der er en fortsat lille vækst i bestanden af plug-in-bybrid varebiler.
...
figur6_1 <- data_nysalg_hus_1_stjerne |>
  ...
  hc_tooltip(shared = TRUE,             pointFormat = "{series.name}: <b>{point.percentage:.1f}</b> pct. (<b>{point.antal:,.1f}</b> varebiler)<br>") %>%
