...
ny_figur_3 <- data_bil %>%
  mutate(
    ...
    ultimo = case_when(
      maaned == 12 ~ paste("Ultimo", dansk_måned_uden, as.character(aar)), 
      TRUE ~ paste("Ultimo", dansk_måned_uden, as.character(aar))
    )
  )
...
figur6.2 <- data_nysalg_erhverv_1 |>
  ...
  hc_chart(marginBottom = 180)  %>%
  hc_tooltip(shared = TRUE,
             pointFormat = "<b>{series.name}</b>: {point.percentage:.1f} pct. (<b>{point.antal:,.0f}</b> varebiler)<br>") %>%
...
Bestanden af varebiler i husholdningerne er faldet med `r abs(round((totalbestand_hus - totalbestand_20_hus)/totalbestand_20_hus * 100, 1))` pct. siden 2020, jf. figur 8.1. Herudover ses det, at el-varebiler næsten ingen andel af bestanden af varebiler i husholdningen, mens bestanden af fossilelvarebiler udgør `r round(fostotalbestand_hus/totalbestand_hus * 100, 1)` pct. Bestanden af varebiler i erhvervet er nogenlunde konstant siden 2020, jf. figur 8.2, hvor bestanden er vokset med `r totalbestand_erhverv - totalbestand_20_erhverv` varebiler. Endeligt ses der en vækst i andelen af elvarebiler i erhvervet. Foreløbigt i `r as.character(nyeste_aar)` udgør de `r round(eltotalbestand_erhverv/totalbestand_erhverv * 100, 1)`  pct. af den samlede bestand i erhvervet, mens fossilvarebiler udgør `r round(fostotalbestand_erhverv/totalbestand_erhverv * 100, 1)` pct.
```{r}
