...
ny_figur_3 <- data_bil %>%
  mutate(
    ...
    rigtigt_år = case_when(
      maaned == 12 ~ paste(aar + 1),   # Treat December as Jan next year
      TRUE ~ paste(dansk_måned_uden, aar)
    )
  )
...
figur_2.1 <- data_nysalg_1_stjerne |> 
  ...
  hc_tooltip(shared = TRUE) %>% 
  format_chart(
    titel = str_glue("<b>Figur 2.1.</b> Drivmidlernes andel af nyregistreringer for personbiler"),
    "Kilde: Danmarks Statistik<br>*År til dato"
  )
...
figur_2.2 <- brugt_1_stjerne |> 
  ...
  hc_tooltip(shared = TRUE) %>% 
  format_chart(
    titel = str_glue("<b>Figur 2.2.</b> Drivmidlernes andel af brugtimport for personbiler"),
    "Kilde: Bilstatistik <br>*År til dato"
  )
