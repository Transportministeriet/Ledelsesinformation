...
brugtvognsimport_data <- dir(sti) %>% 
  map_dfr(function(y) {
    ...
    skip_l <- pull(temp, 2) %>% {.[which(is.na(.))]} %>% length()
    ...
    til_out <- read_excel(
      ...
      select(DRIVKRAFT, ANTAL=ANTAL_3, ANDEL = ANDEL_4) %>% 
...
#Data til figur 4. Status for antallet af elbiler 
...
data_fig_4 <- data_bil_1 |> 
  mutate(DRIV_1 = ifelse(DRIV %in% c("Benzin", "Diesel"), "Fossil", DRIV) |> 
           factor(levels = c("El", "Plug-in-hybrid", "Fossil", "Drivmidler i alt")),
         AAR = year(DATE)) |> 
  summarise(INDHOLD = sum(INDHOLD),
...
tal_fos_2019 <- tal |> filter(DRIV_1=="Fossil", AAR=="2020") |> pull("UDV_ANTAL_BIL")
tal_fos_2023 <- tal |> filter(DRIV_1=="Fossil", AAR==nyeste_aar) |> pull("UDV_ANTAL_BIL")
...
brugt_import_2026 <- brugt_1_stjerne |> 
  filter(AAR == "2026*") |>  # Filter only for 2024
  summarise(total_ANTAL = sum(ANTAL, na.rm = TRUE)) |>  # Sum ANTAL
  pull(total_ANTAL)  # Extract the summed value

brugt_import_2026_EL <- brugt_1_stjerne |> 
  filter(AAR == "2026*", DRIV == "El") |>  # Filter only for 2024
  summarise(total_ANTAL = sum(ANTAL, na.rm = TRUE)) |>  # Sum ANTAL
  pull(total_ANTAL)  # Extract the summed value

brugt_import_andel_2026_EL <- brugt_1_stjerne |> 
  filter(AAR == "2026*", DRIV == "El") |>  # Filter only for 2024
...
farver <- c("groen",  "orange", "blaa", "cyan", "roed") %>% factor(., levels = .) %>% sort(decreasing = T)
...
y_maks <- data_nysalg_2_stjerne |> 
  filter(year > max(year, na.rm = TRUE) - antal_aar_bagud | is.na(year)) |> 
...
data_figur_1_2 <- data_nysalg_2_stjerne |> 
  bind_rows(brugt_1_stjerne %>% rename("A_INDHOLD"="A_NYREG")) %>% 
  select(names(data_nysalg_2_stjerne)) %>% 
  filter(year > max(year, na.rm = TRUE) - antal_aar_bagud | is.na(year)) %>% 
  summarise(INDHOLD = sum(INDHOLD),
            .by = c("AAR", "DRIV")) %>% 
  mutate(A_INDHOLD = round(INDHOLD/sum(INDHOLD)*100,1),
...
elbil_salg <- data_figur_1_2 %>% filter(DRIV=="El") %>% select(AAR, INDHOLD) %>% deframe()
total_salg <- data_figur_1_2 %>% summarise(INDHOLD = sum(INDHOLD), .by = "AAR") %>% select(AAR, INDHOLD) %>% deframe()

naest_ny_aar_salg <- total_salg[as.character(næst_nyeste_aar)]
naest_ny_aar_salg <- total_salg[which(max(total_salg)==total_salg)]

indv_aar_saml_el <- elbil_salg[str_detect(names(elbil_salg), str_glue("^{nyeste_aar}") )]
indv_aar_saml <- total_salg[str_detect(names(total_salg), str_glue("^{nyeste_aar}") )]
...
Siden 2022 er det samlede salg (nyregistreringer + brugtimport) af personbiler steget i alle år og toppede i `r as.character(names(total_salg[which(total_salg==max(total_salg))]))`. I `r as.character(næst_nyeste_aar)` var det samlede salg `r total_salg[as.character(næst_nyeste_aar)]` biler, hvoraf elbiler udgjorde `r elbil_salg[as.character(næst_nyeste_aar)]` biler. I `r as.character(nyeste_aar)` er der samlet set solgt `r indv_aar_saml_el` elbiler.
...
data_figur_1_2 %>% 
  ...
  trm_hc_format("<b>Figur 1.</b> Udvikling i samlet salg af personbiler fordelt på drivmiddel",
...
Fra 2024 til 2025 steg brugtvognsimporten med `r (brugt_import_2025-brugt_import_2024)/brugt_import_2024*100` pct. særligt drevet af en øget brugtimport af elbiler. Der blev således importeret `r brugt_import_2025_EL` brugte elbiler i 2025. I `r nyeste_aar` er der foreløbigt importeret `r brugt_import_2026` biler heraf `r brugt_import_2026_EL` elbiler, jf. figur 1.2.
...
Andelen af elbiler i det samlede salg har også været støt stigende siden 2021, hvor andelen var `r  elbil_salg["2021"]/total_salg["2021"]*100`. I `r as.character(næst_nyeste_aar)` var salgsandelen  oppe på `r  elbil_salg[as.character(næst_nyeste_aar)]/naest_ny_aar_salg*100` pct. I `r nyeste_aar` har andelen af elbiler ligget på `r indv_aar_saml_el/indv_aar_saml*100` pct.
...
data_figur_1_2 %>% 
  ...
  hc_tooltip(shared = TRUE) %>% 
  trm_hc_format(
    titel = "<b>Figur 2.</b> Drivmidlernes andel af samlet salg for personbiler",
    note = tekst_note_nyreg)
...
Elbiler har generelt udgjort en større andel af brugtvognsimporten. I `r nyeste_aar` har elbiler hidtil udgjort `r brugt_import_andel_2026_EL` pct. af brugtvognsimporten, jf. figur 2.2.
...
rest <- KF_tal$INDHOLD[[1]] - DST_tal$INDHOLD[[2]]


```

Jævnfør Klimastatus og -fremskrivning 2026 vil der ved udgangen af 2026 være `r KF_tal$INDHOLD[[1]]` elbiler i Danmark. Bestanden pr. `r str_remove_all(DST_tal$x_axis[[2]], " observeret")` var `r DST_tal$INDHOLD[[2]]` elbiler, jf. figur 4. Differencen mellem det nuværende antal elbiler og det forventede antal elbiler ved udgangen af `r nyeste_aar` er således `r rest`.
...
Af figuren fremgår det yderligere, at væksten i bestanden af plug-in-hybridbiler foreløbig er toppet i `r sprintf("%.0f", max_year_P_I_H)`.
...
data_fig_4 |> 
  ...
  hc_colors(c(trm_colors(c("groen", "orange", "blaa" ,"cyan")))) |> 
  hc_yAxis(title = list(text = "Antal (tusind)"),
           labels = list(
             formatter = JS("function() {
...
Figur 6 viser udvikligen i bestanden over måneder. I `r md_d_afvikles_hurtigere` ud af årets første `r month(max(dat_fig_3$DATE))` måneder er  bestanden af dieselbiler blevet afviklet hurtigere, end det var tilfældet i `r as.character(næst_nyeste_aar)`. Bestanden af benzinbiler er afviklet hurtigere end i `r md_b_afvikles_hurtigere`  ud af de første  `r month(max(dat_fig_3$DATE))` måneder af 2023. Hidtil er `r max_måned_afvikling_b` den måned, hvor bestanden af benzinbiler er faldet mest. 
...
expr <- parse(text =str_c("map(sidste_par_aar$DRIV |> unique(), 
    ~{hchart(sidste_par_aar |> filter(DRIV == .x), 'line', hcaes(x = month.abb[MD], y = UDV_BIL, group = TYPE ), marker = F) |> 
        hc_colors(c(trm_colors(c('cyan', 'orange')))) |> 
