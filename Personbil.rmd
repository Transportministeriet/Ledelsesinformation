...
DST_tal <- data_bil_1 |>
  filter(DRIV == "El", 
         DATE_1 %in% c(max(DATE_1), floor_date(max(DATE_1) - years(1), "year"))) |> 
  mutate(x_axis = ifelse(DATE_1 == max(DATE_1),
                         str_c("1. ", max_måned, " ", nyeste_aar, "<br> observeret (DST)"),  # Line break added
                         str_c("1. januar ", year(DATE_1), "<br> observeret (DST)")),        # Line break added
         oprindelse = "DST")
...
KF_tal <- KF_bestand %>% 
  ...
  mutate(aar = parse_number(aar),
         x_axis = str_glue("1. januar {aar+1} <br> forventet (KF24)"),  # Added <br> for line break
         oprindelse = "KF") %>% 
