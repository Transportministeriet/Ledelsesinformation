...
nyreg_data_1 %>%
  ...
  hc_xAxis(title = list(text = NULL),
           categories = c("2019","2020","2021","2022","2023","2024",paste0(nyeste_aar, stjerne),paste0(seneste_md, " ", nyeste_aar))
           # labels = list(
           #   formatter = JS("
           #     function() {
           #       if (this.isLast) {
...
Diesel er stadig det dominernde drivmiddel i bestanden af lastbiler. Bestanden af lastbiler har generelt været svagt stigende siden 1. januar 2021, hvor den samlede bestand var `r bestand_2021`, mens  bestanden ultimo `r str_c(seneste_md)` 2025 var `r bestand_seneste` lastbiler. 

I `r format(ISOdate(2000, nummer_sidste_md, 1), "%B")` måned udviklede bestanden af ellastbiler sig med `r udv_elbestand_seneste_måned` lastbiler således, at der ultimo `r str_c(seneste_md)` 2025 var `r elbestand_seneste` ellastbiler i Danmark svarende til `r elandel_seneste` pct. af den samlede bestand jf. figur 3.
...
Ifølge Klimastatus og -fremskrvining 2024 forventes der at være cirka `r KF_tal$ANTAL[[2]]` elektriske lastbiler i Danmark ved udgangen af 2025. Differencen mellem det nuværende antal, jævnfør figur 3, og det forventede antal elektriske lastbiler ved udgangen af 2025, jævnfør figur 4, er således `r rest` lastbiler.
...
bestand_data_1 %>% 
  ...
  mutate(x_axis = case_when(
    AAR == "2025" ~ str_c(seneste_md, " ", nyeste_aar),  # Replace Ultimo 2025 with Ultimo seneste_md nyeste_aar
    AAR == "2024" ~ str_c(AAR),  # Keep Ultimo 2024
    str_detect(AAR, "\\(") ~ str_c(seneste_md, " ", nyeste_aar),  # Ensure other cases follow the same format
    TRUE ~ str_c(AAR)
  ),
  oprindelse = "Bilstatistik",
  ANTAL,
