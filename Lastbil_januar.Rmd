...
bestand_data_1 %>% 
  ...
  trm_hc_format(str_glue("<b>Figur 4.</b> Status for antallet af ellastbiler i 2025 - oberserveret versus forventet"),
               note = "Kilde: Bilstatistik")
...
map(udvalgte_kat, ~{nyreg_data_underkat |>
  ...
  trm_hc_format(str_glue("<b>{str_replace_all(.x, c('_'=' ', 'OE'='Ø', 'AA'='Å', 'AE'='Æ')) %>% str_to_sentence()}<b>"), note="Kilde: Bilstatistik")}) |> 
...
map(udvalgte_kat, ~{bestand_data_underkat |> 
 ...
 hc_legend(
  ...
  trm_hc_format(str_glue("<b>{str_replace_all(.x, c('_'=' ', 'OE'='Ø', 'AA'='Å', 'AE'='Æ')) %>% str_to_sentence()}<b>"),note="Kilde: Bilstatisitk")}) |> 
  hw_grid(rowheight = 400) %>% htmltools::browsable()
