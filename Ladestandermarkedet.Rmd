...
Dette er Transportministeriets interne statusnotat for ladeinfrastruktur, som viser udviklingen i antallet af ladepunkter. Notatet opdateres månedligt er baseret på data fra ChargeX og Danmarks Statistik.  

Ønskes at arbejde med data fra rapporten kan de hentes her:
```{r}
list(
  "Figur 1" = figur_1 %>% 
    select(`Måned (ultimo)` = DATE, Ladepunkter =  OUTLETS, `Ladepunkter (udvikling)` =  UDV_OUTLETS), 
  "Figur 2" = figur_2  %>% 
    select(`Måned (ultimo)` = DATE, Ladeeffekt = LADEEFFEKT, Ladepunkter =  OUTLETS) %>% 
    pivot_wider(names_from = "Ladeeffekt", values_from = "Ladepunkter"), 
  "Figur 3" = figur_3 %>% 
...
highchart() %>%  
  ...
  hc_add_series(figur_1, "column", yAxis = 1,  hcaes(x = DATE, y =UDV_OUTLETS), color = trm_colors("orange"), name = "Antal nye ladepunkter (Højre akse)", borderColor = 0) %>% 
