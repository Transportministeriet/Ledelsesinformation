...
knit_hooks$set(inline = function(x) {
  if(is.numeric(x)){
  return(prettyNum(x, big.mark=".", digits = 0))
  } else{
    return(x)
  }
})
...
Den faktiske udvikling i antallet af elbiler går meget hurtigere end hvad KF23 har estimeret for 2023. Den observerede tilgang af elbiler er på godt `r antal_el_biler_ialt` biler, svarende til knap 84 pct. højere end estimeret i KF23 for samme periode, jf. figur 1. Elbilers tilgang i 2023 har i snit været 5.600 biler pr. måned, mens KF23 estimerer et snit på godt 2.900 biler. KF23 estimerer, at dette års udvikling i tilgangen af elbiler, først ville indtræffe i 2027, hvor den månedlige tilgang forventes at være godt 5.800 biler pr. måned jf. KF23.
