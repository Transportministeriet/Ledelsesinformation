...
result_max_month <- nyreg_data_3 %>%
  ...
  summarize(
    ANTAL = sum(ANTAL),                  
    DATE = max(DATE),                    # Keep the maximum DATE for each group
    .groups = 'drop'
  ) %>% 
  group_by(AAR, MONTH, ANVENDELSE) %>%            
  mutate(
    A_NYREG = ANTAL / sum(ANTAL) * 100,  
    AAR = str_c(year(DATE), " (", lubridate::quarter(DATE), ". Kvt)")  # Use DATE to create AAR with year and quarter
  ) %>%  
  ungroup()
...
nyreg_data_2 |> 
  ...
  trm_hc_format("<b>Figur 1.</b> Udvikling i nyregistreringer af busser fordelt på drivmiddel",
...
Den 1. januar 2020 var den samlede bestand på `r bestand_2020` busser. Fra 2020 til `r as.character(nyeste_aar_efter)` har bestanden været let faldende, så bestanden pr. `r str_c("1. ", format(ISOdate(2000, 1:12, 1), "%B")[as.numeric(nummer_md_efter)], " ", nyeste_aar_efter)` var `r bestand_seneste` busser. Den `r str_c("1. ", format(ISOdate(2000, 1:12, 1), "%B")[as.numeric(nummer_md_efter)], " ", nyeste_aar_efter)` udgjorde elbusser `r round(bestand_data_x[2, 4], digits = 0)` pct. af den samlede bestand mod 1,2 pct. i 2020. 
...
map(udvalgte_kat, ~{
  nyreg_data_underkat |>
    ...
    hc_legend(reversed = TRUE) |> 
    hc_tooltip(shared = TRUE,
               pointFormat = "<b>{series.name}</b>: {point.percentage:.1f} pct. ({point.y:,.0f} busser) <br>") %>% 
    trm_hc_format(
      str_glue("<b>{.x}</b>"),
      note = str_glue("Kilde: Bilstatistik<br>{stjerne_tekst}")
    )
...
map(udvalgte_kat, ~{bestand_data_underkat |> 
    ...
    hc_tooltip(
      shared = TRUE,
      pointFormat = "<b>{series.name}</b>: {point.y:,.0f} busser ({point.percentage:.1f} pct.) <br>"
    ) |> 
    hc_legend(reversed = TRUE) |> 
    trm_hc_format(
      str_glue("<b>{.x}</b>"),
