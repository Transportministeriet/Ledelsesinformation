...
til_indlaes <- bilstat_peri <- dir(str_c("S:/TRM Databank/004 Bilstatistik/Bus")) %>% 
  enframe() %>% 
  mutate(
    value_std = str_replace_all(value, "\\.", "-"),
    dato = coalesce(
      parse_date_time2(value_std, orders = "dmY") %>% as.Date(),
      map_chr(value_std, ~strsplit(.x, ", ", fixed = TRUE)[[1]][2]) %>% 
        map_chr(~strsplit(.x, "- ", fixed = TRUE)[[1]][2]) %>% 
        parse_date_time2(orders = "dmY") %>% 
        as.Date()
    ),
  ...
  filter(
    !is.na(dato),
    month(dato) == max(month(dato), na.rm = TRUE),
    .by = c("aar", "type")
  )
...
bestand_data <- til_indlaes %>% 
  ...
  enframe() %>% 
  mutate(
    dato = str_extract(value, "\\d{2}[.-]\\d{2}[.-]\\d{4}") %>% 
      str_replace_all("\\.", "-") %>% 
      parse_date(format = "%d-%m-%Y"),
    aar = year(dato)
  ) %>% 
  filter(dato == max(dato), .by = "aar") %>% 
  pull(value) %>% 
  (\(y) map_dfr(
    y,
    ~read_excel(
      ...
      mutate(
        DRIVKRAFT = str_extract_all(DRIVKRAFT, "PHEV|El", simplify = TRUE) %>% 
          na_if("") %>% 
          coalesce(DRIVKRAFT),
        DRIV = ifelse(
          DRIVKRAFT %in% c("Benzin", "Gas", "PHEV", "Brint", "Ukendt"),
          "Øvrige drivmidler",
          DRIVKRAFT
        ) %>% factor(levels = c("Øvrige drivmidler", "Diesel", "El")),
        DATE = str_extract(.x, "\\d{2}[.-]\\d{2}[.-]\\d{4}") %>% 
          str_replace_all("\\.", "-") %>% 
          dmy()
      )
  ))()
...
farver <- c("roed",  "blaa", "groen") %>% factor(., levels = .)
